...
#else
      sprintf(tempbuf, "%d", (*pArrInfo)[i].trackid);
      ptree_face.put("trackid", tempbuf);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", (*pArrInfo)[i].x);
      ptree_face.put("x", tempbuf);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", (*pArrInfo)[i].y);
      ptree_face.put("y", tempbuf);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", (*pArrInfo)[i].width);
      ptree_face.put("width", tempbuf);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", (*pArrInfo)[i].hight);
      ptree_face.put("hight", tempbuf);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", (*pArrInfo)[i].face_quality_score);
      ptree_face.put("facescore", tempbuf);
      ptree_face.put("imgbase64data", (*pArrInfo)[i].img_base64_data);
      ptree_face.put("gender", (*pArrInfo)[i].gender);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", (*pArrInfo)[i].age);
      ptree_face.put("age", tempbuf);
      ptree_face.put("face_feature", (*pArrInfo)[i].face_feature);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", (*pArrInfo)[i].horizontal_angle);
      ptree_face.put("horizontal_angle", tempbuf);

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", (*pArrInfo)[i].vertical_angle);
      ptree_face.put("vertical_angle", tempbuf);

      // 人脸所占摄像头视角的比例
      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", (*pArrInfo)[i].horizontal_ratio);
      ptree_face.put("horizontal_ratio", tempbuf);

      memset(tempbuf, '\0',sizeof(tempbuf));
      sprintf(tempbuf, "%f", (*pArrInfo)[i].vertical_ratio);
      ptree_face.put("vertical_ratio", tempbuf);

      // 估计的人脸的距离
      memset(tempbuf, '\0',sizeof(tempbuf));
      sprintf(tempbuf, "%f", (*pArrInfo)[i].estimate_distance);
      ptree_face.put("estimate_distance", tempbuf);

      memset(tempbuf, '\0',sizeof(tempbuf));
      sprintf(tempbuf, "%d", (*pArrInfo)[i].serial_num);
      ptree_face.put("serial_num", tempbuf);
#endif
...
#else
  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%d", face_info.trackid);
  ptree_face["trackid"] =tempbuf;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%d", face_info.x);
  ptree_face["x"] =tempbuf;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%d", face_info.y);
  ptree_face["y"] =tempbuf;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%d", face_info.width);
  ptree_face["width"] =tempbuf;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%d", face_info.hight);
  ptree_face["hight"] =tempbuf;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%f", face_info.face_quality_score);
  ptree_face["facescore"] = tempbuf;
  ptree_face["imgbase64data"]= face_info.img_base64_data;
  ptree_face["gender"]= face_info.gender;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%d",face_info.age);
  ptree_face["age"]= tempbuf;
  ptree_face["face_feature"]= face_info.face_feature;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%f", face_info.horizontal_angle);
  ptree_face["horizontal_angle"]= tempbuf;

  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%f", face_info.vertical_angle);
  ptree_face["vertical_angle"]= tempbuf;

  // 人脸所占摄像头视角的比例
  memset(tempbuf, '\0', sizeof(tempbuf));
  sprintf(tempbuf, "%f", face_info.horizontal_ratio);
  ptree_face["horizontal_ratio"] = tempbuf;

  memset(tempbuf, '\0',sizeof(tempbuf));
  sprintf(tempbuf, "%f", face_info.vertical_ratio);
  ptree_face["vertical_ratio"] = tempbuf;

  // 估计的人脸的距离
  memset(tempbuf, '\0',sizeof(tempbuf));
  sprintf(tempbuf, "%f", face_info.estimate_distance);
  ptree_face["estimate_distance"]= tempbuf;

  memset(tempbuf, '\0',sizeof(tempbuf));
  sprintf(tempbuf, "%d", face_info.serial_num);
  ptree_face["serial_num"] =tempbuf;
#endif  
  return YSOS_ERROR_SUCCESS;
}
...
#else
    sprintf(tempbuf, "%s", pComarisonResult->group_id.c_str());
    ptree_root.put("groupId", tempbuf);
#endif
...
#else
  sprintf(tempbuf, "%d", puint->have_perfect_face_);
  json_object["have_perfect_face"] = std::string(tempbuf);
#endif
...
#else
      sprintf(tempbuf, "%d", detect_ptr->trackid);
      face_json["trackid"] = tempbuf;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", detect_ptr->x);
      face_json["x"] = tempbuf;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d",detect_ptr->y);
      face_json["y"] = tempbuf;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", detect_ptr->width);
      face_json["width"] = tempbuf;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", detect_ptr->hight);
      face_json["hight"] = tempbuf;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", detect_ptr->face_quality_score);
      face_json["facescore"]= tempbuf;
#endif
...
#else
      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%d", detect_ptr->age);
      face_json["age"] = tempbuf;

      face_json["face_feature"] =detect_ptr->face_feature;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", detect_ptr->horizontal_angle);
      face_json["horizontal_angle"] = tempbuf;

      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", detect_ptr->vertical_angle);
      face_json["vertical_angle"] = tempbuf;

      // 人脸所占摄像头视角的比例
      memset(tempbuf, '\0', sizeof(tempbuf));
      sprintf(tempbuf, "%f", detect_ptr->horizontal_ratio);
      face_json["horizontal_ratio"] = tempbuf;

      memset(tempbuf, '\0',sizeof(tempbuf));
      sprintf(tempbuf, "%f", detect_ptr->vertical_ratio);
      face_json["vertical_ratio"] = tempbuf;

      // 估计的人脸的距离
      memset(tempbuf, '\0',sizeof(tempbuf));
      sprintf(tempbuf, "%f",detect_ptr->estimate_distance);
      face_json["estimate_distance"] = tempbuf;

      memset(tempbuf, '\0',sizeof(tempbuf));
      sprintf(tempbuf, "%d", detect_ptr->serial_num);
      face_json["serial_num"] = tempbuf;
#endif
      // have perfect face
      {
