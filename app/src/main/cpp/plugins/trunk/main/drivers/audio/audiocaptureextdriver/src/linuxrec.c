...
{
	snd_pcm_state_t tstate = snd_pcm_state(pcm);
	printf("start_record_internal---->snd_pcm_state=[%d]\n", tstate);
	if (SND_PCM_STATE_SETUP == tstate) {
	  {
		int ret = snd_pcm_prepare(pcm);
		if ( 0 == ret ) {
			ret = start_record_internal(pcm);
		} else {
			printf("start_record_internal---->snd_pcm_prepare=[%d]\n", ret);
			return ret;
		}
	  }
	} else if ( SND_PCM_STATE_PREPARED == tstate) {
      return snd_pcm_start(pcm);
	} else if ( SND_PCM_STATE_RUNNING == tstate) {
		return 0;
	} else {
		return -1;
	}
...
static int stop_record_internal(snd_pcm_t *pcm)
{
	...
	printf("stop_record_internal---->snd_pcm_state=[%d]\n", state);
		...
		record_dev_id dev, WAVEFORMATEX * fmt)
{
	...
	printf("open_recorder_internal---->snd_pcm_open=[%d]\n", err);
	if(err < 0)
		goto fail;

	err = set_params(rec, fmt, DEF_BUFF_TIME, DEF_PERIOD_TIME);
	printf("open_recorder_internal---->set_params=[%d]\n", err);
	if(err)
		goto fail;

	assert(rec->bufheader == NULL);
	err = prepare_rec_buffer(rec);
	printf("open_recorder_internal---->prepare_rec_buffer=[%d]\n", err);
	if(err)
		goto fail;

	err = create_record_thread((void*)rec, 
			&rec->rec_thread);
	printf("open_recorder_internal---->create_record_thread=[%d]\n", err);
	if(err)
		goto fail;
...
static void close_recorder_internal(struct recorder *rec)
{
	...
	int err = pthread_cancel(rec->rec_thread);
	printf("close_recorder_internal---->pthread_cancel=[%d]\n", err);
...
int open_recorder(struct recorder * rec, record_dev_id dev, WAVEFORMATEX * fmt)
{
	...
	printf("open_recorder---->open_recorder_internal=[%d]\n", ret);
	if(ret == 0)
...
int start_record(struct recorder * rec)
{
	...
	printf("start_record---->start_record_internal=[%d]\n", ret);
	if(ret == 0)
...
int stop_record(struct recorder * rec)
{
	...
	printf("stop_record---->stop_record_internal=[%d]\n", ret);
	if(ret == 0) {		
		rec->state = RECORD_STATE_READY;
	}
	return ret;
