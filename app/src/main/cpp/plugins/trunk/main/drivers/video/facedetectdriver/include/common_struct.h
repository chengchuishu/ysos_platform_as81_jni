...
/**
 * TODO: add for linux
 * @breaf: 将void *转换为原来类型的指针，然后再调用delete释放指针，如果原来的指针是数组指针，那么必须使用delete []删除指向的内存空间
 **/
...
struct FaceInfo {
  ...
  FaceInfo()
      : face_quality(0.f),
        ...
        x(0), y(0),
        width(0), height(0),
        /*ex_data(0), // add for linux */ 
        track_id(-1) {}

  ~FaceInfo() { /*if (ex_data) delete ex_data; //add for linux */ }
};
