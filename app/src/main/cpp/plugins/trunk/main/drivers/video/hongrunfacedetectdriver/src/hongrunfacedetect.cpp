...
/// hongruan1.2 sdk headers
//#include <hongruan/include/merror.h>
//#include <hongruan/include/arcsoft_fsdk_face_detection.h>
//#include <hongruan/include/arcsoft_fsdk_age_estimation.h>
//#include <hongruan/include/arcsoft_fsdk_gender_estimation.h>
//#include <hongruan/include/arcsoft_fsdk_face_recognition.h>
//#include <hongruan/include/arcsoft_fsdk_face_tracking.h>
...
#define WORKBUF_SIZE_FR        (40*1024*1024)
	...
	int FaceDetectComparisonHR::InitFDEngine() {
		...
		nRet = ASFOnlineActivation((MPChar)hr_appid_.c_str(), (MPChar)hr_fd_sdkkey_.c_str());
		if (nRet != MOK && nRet != MERR_ASF_ALREADY_ACTIVATED) {
			YSOS_LOG_ERROR_CUSTOM(MY_LOGGER, "AFD_FSDK_ ASFOnlineActivation fail, ret:" << nRet);
		...
		else
		{
			...
			nRet = ASFGetActiveFileInfo(&activeFileInfo);
			if (nRet != MOK)
			{
				YSOS_LOG_ERROR_CUSTOM(MY_LOGGER, "ASFGetActiveFileInfo fail: [" << nRet << "]");
			}


			/// 初始化引擎
			MInt32 mask = ASF_FACE_DETECT | ASF_FACERECOGNITION | ASF_AGE | ASF_GENDER | ASF_FACE3DANGLE | ASF_LIVENESS;
			nRet = ASFInitEngine(ASF_DETECT_MODE_IMAGE, ASF_OP_0_ONLY, 32, 5, mask, &hr_facedetect_engine_handle_);	  //ASFInitEngine(ASF_DETECT_MODE_IMAGE, ASF_OP_0_ONLY, NSCALE, FACENUM, mask, &handle);
			if (nRet != MOK) {
	...
	//裁剪图片,做四字节对齐
	void FaceDetectComparisonHR::CutIplImage(IplImage* src, IplImage* dst, int x, int y)
	{
	...
	int FaceDetectComparisonHR::UnInitFDEngine() {
		...
		if (hr_facedetect_engine_handle_) {
			MRESULT nRet = MERR_UNKNOWN;
			nRet = ASFUninitEngine(hr_facedetect_engine_handle_);
			if (nRet != MOK) {
	...
	*/
	...
	int FaceDetectComparisonHR::Comparison11Local(const char* face_feature_a, const int face_feature_a_length,
		...
		ASF_FaceFeature facemode1 = { 0 }, facemode2 = { 0 };
		facemode1.feature = (MByte *)face_feature_a;
		facemode1.featureSize = face_feature_a_length;
		facemode2.feature = (MByte *)face_feature_b;
		facemode2.featureSize = face_feature_b_length;
		nRet = ASFFaceFeatureCompare(hr_facedetect_engine_handle_, &facemode1, &facemode2, &scores);
	...
	int FaceDetectComparisonHR::FaceDetect(const char* pdata, const int& width, const int& height, const int& channels,
		...
		/// 人脸检测
		MRESULT nRet = MERR_UNKNOWN;
		ASF_MultiFaceInfo fd_faceres = { 0 };
		ASF_MultiFaceInfo ft_faceres = { 0 };
		
		cv::Mat srcFrame(int(height), int(width), CV_8UC3, (void*)pdata);
		//IplImage *lpSrc = &srcFrame;
		IplImage* cutImg1 = cvCreateImage(cvSize( width -  width % 4,  height), IPL_DEPTH_8U, channels);
        IplImage tmp_image = IplImage(srcFrame); //临时对象就是无名对象，在语句行上直接用类生成一个对象，它的生命周期是在该行创建，又在该行销毁.
		CutIplImage(&tmp_image, cutImg1, 0, 0);

		if (/*need_track*/false) { ///<  按视频方式 检测
		  /* 人脸跟踪 */
			nRet = ASFDetectFaces(hr_facedetect_engine_handle_, offInput.i32Width, offInput.i32Height, ASVL_PAF_RGB24_B8G8R8, offInput.ppu8Plane[0], &fd_faceres);
			...
			/// alloc meory ret
			for (int i = 0; i < fd_faceres.faceNum; ++i) {
			...
			// copy data offInput.i32Width
			for (int i = 0; i < ft_faceres.faceNum; ++i) {
				vec_face_detect_info[i]->trackid = i;
				vec_face_detect_info[i]->x = ft_faceres.faceRect[i].left - 20 <= 0 ? 0 : ft_faceres.faceRect[i].left - 20;
				vec_face_detect_info[i]->y = ft_faceres.faceRect[i].top - 20 <= 0 ? 0 : ft_faceres.faceRect[i].top - 20;
				vec_face_detect_info[i]->width = ft_faceres.faceRect[i].right + 20 >= offInput.i32Width ? offInput.i32Width - ft_faceres.faceRect[i].left : abs(ft_faceres.faceRect[i].right - ft_faceres.faceRect[i].left) + 20;
				vec_face_detect_info[i]->hight = ft_faceres.faceRect[i].bottom + 20 >= offInput.i32Height ? offInput.i32Height - ft_faceres.faceRect[i].bottom : abs(ft_faceres.faceRect[i].bottom - ft_faceres.faceRect[i].top) + 20;

				switch (ft_faceres.faceOrient[i]) {
				case ASF_OC_0:
			...
			nRet = ASFDetectFaces(hr_facedetect_engine_handle_, cutImg1->width, cutImg1->height, ASVL_PAF_RGB24_B8G8R8, (MUInt8*)cutImg1->imageData, &fd_faceres);
			...
			for (int i = 0; i < fd_faceres.faceNum; ++i) {
				...
				vec_face_detect_info[i]->x = fd_faceres.faceRect[i].left - 40 <= 0 ? 0 : fd_faceres.faceRect[i].left - 40;
				vec_face_detect_info[i]->y = fd_faceres.faceRect[i].top - 40 <= 0 ? 0 : fd_faceres.faceRect[i].top - 40;
				vec_face_detect_info[i]->width = abs(fd_faceres.faceRect[i].right + 40) >= cutImg1->width ? abs(cutImg1->width - fd_faceres.faceRect[i].left) : abs(fd_faceres.faceRect[i].right - fd_faceres.faceRect[i].left) + 40;
				vec_face_detect_info[i]->hight = abs(fd_faceres.faceRect[i].bottom + 40) >= cutImg1->height ? abs(cutImg1->height - fd_faceres.faceRect[i].top) : abs(fd_faceres.faceRect[i].bottom - fd_faceres.faceRect[i].top) + 40;

				switch (fd_faceres.faceOrient[i]) {
				case ASF_OC_0:
					vec_face_detect_info[i]->face_detect_detail_ptr->headPose.yaw = 0;
					break;   // 0 degree
				case ASF_OC_90:
		...
		if (need_face_attribute && need_track) { ///< 视频方式的才估算距离
		  ...
		  //float max_horizon_angle_half =0, max_vertical_angle_half =0, oriented_with_horizontal_angle=0, middle_offset_distance=0;
			double x_max_width = cutImg1->width / 2;
			double y_max_hegiht = cutImg1->height / 2;
			for (unsigned int i = 0; i < vec_face_detect_info.size(); ++i) {
		...
		if (NULL != pdata)
		{
			cv::Mat frame(int(cutImg1->height), int(cutImg1->width), data_format, (void*)pdata);
			for (unsigned int i = 0; i < vec_face_detect_info.size(); ++i) {
				...
				cv::Mat roi_frame(frame, cv::Rect(vec_face_detect_info[i]->x, vec_face_detect_info[i]->y, \
					vec_face_detect_info[i]->width + vec_face_detect_info[i]->x > width ? width - vec_face_detect_info[i]->x : vec_face_detect_info[i]->width, \
					vec_face_detect_info[i]->hight + vec_face_detect_info[i]->y > height ? height - vec_face_detect_info[i]->y : vec_face_detect_info[i]->hight));
				vec_face_mat.push_back(roi_frame.clone());
			}
		}
		else
		{
			YSOS_LOG_ERROR_CUSTOM(MY_LOGGER, " pdata  is null ");
			return YSOS_ERROR_FAILED;
		}
		...
		if (get_face_image_base64) { ///< 需自己提取
			...
			for (unsigned int i = 0; i < vec_face_detect_info.size(); ++i) {
				...
				//if (mat_single_face.data)
				{
					...
					YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, "vecImg.size():[" << vecImg.size() << "]");
				}
			}
		}
		YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " need_face_attribute&&  hr_facedetect_engine_handle_");
		if (need_face_attribute&&  hr_facedetect_engine_handle_) {
			//g_Log.Trace_Print(TraceLogLevel_DEBUG, __FILE__, __LINE__, " 开始 获取 人脸属性" );
			//MInt32 processMask = ASF_AGE | ASF_GENDER | ASF_FACE3DANGLE;
			//nRet = ASFProcess(hr_facedetect_engine_handle_, width, height, ASVL_PAF_RGB24_B8G8R8, (MUInt8 *)pdata, &ft_faceres, processMask);
			//nRet = ASFProcess(hr_facedetect_engine_handle_, offInput.i32Width,offInput.i32Height,  ASVL_PAF_RGB24_B8G8R8, (MUInt8 *)offInput.ppu8Plane[0], &fd_faceres,processMask);
			processMask = ASF_AGE | ASF_GENDER | ASF_FACE3DANGLE;
			YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFProcess begin");
			nRet = ASFProcess(hr_facedetect_engine_handle_, cutImg1->width, cutImg1->height, ASVL_PAF_RGB24_B8G8R8, (MUInt8 *)cutImg1->imageData, &fd_faceres, processMask);
			YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFProcess End nRet=[" << nRet << "]");
			if (MOK != nRet && MERR_FSDK_FACEFEATURE_LOW_CONFIDENCE_LEVEL != nRet) {
				YSOS_LOG_ERROR_CUSTOM(MY_LOGGER, "Face Detection failed, error code:" << nRet);
				return YSOS_ERROR_FAILED;
			}
			if (MERR_FSDK_FACEFEATURE_LOW_CONFIDENCE_LEVEL != nRet) { //人脸特征检测结果置信度低时不取特征
				ASF_GenderInfo genderInfo = { 0 };
				if (/*need_track*/false) { ///< 按视频方式 检测
					nRet = ASFGetGender(hr_facedetect_engine_handle_, &genderInfo);

				}
				else {    ///< 按静态图片的方式检测
					YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFGetGender begin");
					nRet = ASFGetGender(hr_facedetect_engine_handle_, &genderInfo);
					YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFGetGender End");
				}
				if (MOK == nRet) {
					for (int i = 0; i < genderInfo.num; ++i) {
						/// "1" represents female, "0" represents male, and "-1" represents unknown.
						switch (genderInfo.genderArray[i]) {
						case 1:
							vec_face_detect_info[i]->gender = "-1";
							break;
						case 0:
				...
				YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFGetAge Begin");
				ASF_AgeInfo ageInfo = { 0 };
				if (/*need_track*/false) { ///< 按视频方式 检测
					nRet = ASFGetAge(hr_facedetect_engine_handle_, &ageInfo);
				}
				else {    ///< 按静态图片检测
					nRet = ASFGetAge(hr_facedetect_engine_handle_, &ageInfo);
				}
				YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFGetAge End");
				if (MOK == nRet) {
					for (int i = 0; i < ageInfo.num; ++i) {
						vec_face_detect_info[i]->age = ageInfo.ageArray[i];  ///< "0" represents unknown,
					}
				}
				else {
					YSOS_LOG_ERROR_CUSTOM(MY_LOGGER, "error, age estimation error, retcode:" << nRet);
		...
		YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " 人脸特征提取");
		if (/*need_track*/false && (get_field_feature || get_prob_feature) && hr_facedetect_engine_handle_) { ///< 人脸特征提取
			YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " 人脸特征提取 Begin");
			for (int i = 0; i < ft_faceres.faceNum; ++i) {
				ASF_SingleFaceInfo faceInput;
				//第一张人脸信息通过face detection\face tracking获得
				faceInput.faceOrient = ft_faceres.faceOrient[i];//人脸方向
				//人脸框位置
				faceInput.faceRect.left = ft_faceres.faceRect[i].left;
				faceInput.faceRect.top = ft_faceres.faceRect[i].top;
				faceInput.faceRect.right = ft_faceres.faceRect[i].right;
				faceInput.faceRect.bottom = ft_faceres.faceRect[i].bottom;

				/// 提取人脸特征
				ASF_FaceFeature LocalFaceModels = { 0 };
				//nRet = ASFFaceFeatureExtract(hr_facerecognition_engine_handle_, &offInput, &faceInput, &LocalFaceModels);
				YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFFaceFeatureExtract Begin");
				nRet = ASFFaceFeatureExtract(hr_facedetect_engine_handle_, \
					cutImg1->width, \
					cutImg1->height, \
					ASVL_PAF_RGB24_B8G8R8, \
					(MUInt8 *)cutImg1->imageData, \
					&faceInput,
					&LocalFaceModels);
				YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFFaceFeatureExtract End");
				if (MOK != nRet) {
					YSOS_LOG_ERROR_CUSTOM(MY_LOGGER, "fail to Extract FR Feature, error code:" << nRet);
					continue; ///< 提取下一个人脸的特征
				}
				vec_face_detect_info[i]->vec_face_feature.insert(vec_face_detect_info[i]->vec_face_feature.begin(), LocalFaceModels.feature, LocalFaceModels.feature + LocalFaceModels.featureSize);
			}
			YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " 人脸特征提取 End");
		}
		else if ((get_field_feature || get_prob_feature || need_track) && hr_facedetect_engine_handle_) {
			/// 按图片方式检索
			YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " 人脸特征提取 pic Begin");
			for (int i = 0; i < fd_faceres.faceNum; ++i) {
				ASF_SingleFaceInfo faceInput;
				//第一张人脸信息通过face detection\face tracking获得
				faceInput.faceOrient = fd_faceres.faceOrient[i]; //人脸方向
				//人脸框位置
				faceInput.faceRect.left = fd_faceres.faceRect[i].left;
				faceInput.faceRect.top = fd_faceres.faceRect[i].top;
				faceInput.faceRect.right = fd_faceres.faceRect[i].right;
				faceInput.faceRect.bottom = fd_faceres.faceRect[i].bottom;


				/// 提取人脸特征
				YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " ASFFaceFeatureExtract pic End");
				ASF_FaceFeature LocalFaceModels = { 0 };
			...
			YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " 人脸特征提取 pic End");
		}

		YSOS_LOG_DEBUG_CUSTOM(MY_LOGGER, " 更新人脸跟踪号");
		// 更新人脸跟踪号
		if (need_track) {
			...
			for (unsigned int i = 0; i < vec_face_detect_info.size(); ++i) {
				...
				//  continue;
				//}
				if (vec_face_detect_info[i]->width < 100 || vec_face_detect_info[i]->hight < 100)
				{
					continue;
				}
