...
#ifndef NLPEXT_MODULE_H_
#define NLPEXT_MODULE_H_
...
*/
class YSOS_EXPORT NlpExtModule : public BaseModuleImpl {
  DECLARE_CREATEINSTANCE(NlpExtModule);
  DISALLOW_COPY_AND_ASSIGN(NlpExtModule);
  DECLARE_PROTECT_CONSTRUCTOR(NlpExtModule);

 public:
  virtual ~NlpExtModule(void);
  /**
  ...
  */
   ...
   *@return 需要数据转换返回true，否则返回false
   */
  bool NeedConvertDataTypeInfo(const std::string source_input_datatype, const std::string source_output_datatypa, ModuleDataTypeConvertInfo &convert_info);
 ...
 protected:
  ...
  std::string relate_driver_;                                                   //< 关联的driver类
  DriverInterfacePtr driver_prt_;                                               //< driver的指针 //  NOLINT
  LightLock ioctl_lock_;                                                        //< 互斥变量
  LightLock property_lock_;                                                     //< 互斥变量
};

}
#endif    ///NLPEXT_MODULE_H_    //NOLINT