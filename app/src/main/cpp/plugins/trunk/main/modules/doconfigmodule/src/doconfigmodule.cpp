/**
 *@file doconfigmodule.cpp
 *@brief doconfig module
 *@version 1.0
 *@author l.j..
...
#include "../include/doconfigmodule.h"
#include <json/json.h>
#include <boost/filesystem.hpp>

namespace ysos {
DECLARE_PLUGIN_REGISTER_INTERFACE(DoconfigModule, ModuleInterface);  ///<  插件的入口，一定要加上 //  NOLINT

DoconfigModule::DoconfigModule(const std::string &strClassName) : BaseThreadModuleImpl(strClassName) {

  exe_cmd_ = CMD_CONFIG_AUTO;
  thread_data_->timeout = 500;

  logger_ = GetUtility()->GetLogger("ysos.doconfig");
}

DoconfigModule::~DoconfigModule(void) {

}

int DoconfigModule::GetProperty(int type_id, void *type) {
  int n_return = YSOS_ERROR_SUCCESS;
  int n_result = -1;

  do {
    if (type_id == CMD_CONFIG_AUTO) {
      ...
      case CMD_CONFIG_CALLBACKINFO:
        {
          driver_prt_->Ioctl(CMD_CONFIG_CALLBACKINFO, in_buffer_ptr, in_buffer_ptr);
          exe_cmd_ = CMD_CONFIG_AUTO;
          break;
        }
      case CMD_CONFIG_LOG:
        {
          memcpy(buffer, log_info, sizeof(LOG));
          driver_prt_->Ioctl(CMD_CONFIG_LOG, in_buffer_ptr, in_buffer_ptr);
          exe_cmd_ = CMD_CONFIG_AUTO;
          break;
        }
      }
...
int DoconfigModule::Ioctl(INT32 control_id, LPVOID param /* = nullptr */) {
  ...
  do {
    ...
    lock_.Lock();
    switch (control_id) {
    case CMD_CONFIG_ANAL: {

      exe_cmd_ = CMD_CONFIG_CALLBACKINFO;
      break;
    }
    case CMD_CONFIG_ADD: {
      ...
      for (auto itor = js_value.begin(); itor != js_value.end(); ++ itor) {
        std::string key = itor.key().asString();
        std::string value = js_value[itor.key().asString().c_str()].asString();
        YSOS_LOG_DEBUG("key = " << key.c_str());
        YSOS_LOG_DEBUG("value = " << value.c_str());
        //sprintf_s(my_buffer, 100, "%s|%s|", key.c_str(), value.c_str());
        sprintf(my_buffer, "%s|%s|", key.c_str(), value.c_str());//add  for linux 
        YSOS_LOG_DEBUG(my_buffer);
        memcpy(buffer, my_buffer, 100);
        n_return = driver_prt_->Ioctl(CMD_CONFIG_ADD, in_buffer_ptr, in_buffer_ptr);
      }
    ...
    case CMD_CONFIG_MODIFY: {
      ...
      for (auto itor = js_value.begin(); itor != js_value.end(); ++ itor) {
        ...
        YSOS_LOG_DEBUG(my_buffer);
        memcpy(buffer, my_buffer, 100);
        n_return = driver_prt_->Ioctl(CMD_CONFIG_MODIFY, in_buffer_ptr, in_buffer_ptr);
      }
      /*if(n_return == YSOS_ERROR_SUCCESS)
      	exe_cmd_ = CMD_CONFIG_CALLBACKINFO;*/
      break;
    ...
    case CMD_CONFIG_LOG:
      {
        ...
        js_reader.parse(str_param, js_value, true);
        memset(log_info, 0, sizeof(LOG));
        log_info->log_name_ = js_value["name"].asString();
        if(!js_value["level"].isNull())
          log_info->log_level_ = js_value["level"].asString();
        else
          log_info->log_level_ = "ERROR";
        exe_cmd_ = CMD_CONFIG_LOG;
        break;
      }
    default: {
      n_return = YSOS_ERROR_FAILED;
...
int DoconfigModule::Flush(LPVOID param /* = nullptr */) {
  return YSOS_ERROR_SUCCESS;
}

int DoconfigModule::Initialize(LPVOID param /* = nullptr */) {
  ...
  do {
    ...
    driver_prt_ = ysos::GetDriverInterfaceManager()->FindInterface("default@DoconfigDriver");
...
int DoconfigModule::UnInitialize(LPVOID param /* = nullptr */) {
  return BaseThreadModuleImpl::UnInitialize(param);
}

int DoconfigModule::InitalDataInfo(void) {
...
int DoconfigModule::RealOpen(LPVOID param /* = nullptr */) {
  ...
  do {
    ...
    if (n_return != YSOS_ERROR_SUCCESS)
      break;
    BufferInterfacePtr in_buffer_ptr;
    buffer_pool_ptr_->GetBuffer(&in_buffer_ptr);
    driver_prt_->Ioctl(CMD_CONFIG_CALLBACKINFO, in_buffer_ptr, in_buffer_ptr);
...
int DoconfigModule::RealClose(void) {
...
int DoconfigModule::RealPause(void) {
  return YSOS_ERROR_SUCCESS;
}

int DoconfigModule::RealStop(void) {
  return YSOS_ERROR_SUCCESS;
}

int DoconfigModule::RealRun(void) {
  return YSOS_ERROR_SUCCESS;
}

int DoconfigModule::Initialized(const std::string &key, const std::string &value) {
