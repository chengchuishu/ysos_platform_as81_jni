...
*/
    class YSOS_EXPORT TemplateModule : public BaseThreadModuleImpl {
    DECLARE_CREATEINSTANCE(TemplateModule);
    DISALLOW_COPY_AND_ASSIGN(TemplateModule);
    DECLARE_PROTECT_CONSTRUCTOR(TemplateModule);

    public:
        virtual ~TemplateModule(void);
        /**
    ...
    private:
        ...
        int TransferType4Event(const std::string &typedata);
        int Generate4Event(const std::string &eventdata);
        int NotifyEvent4Callback(const std::string &eventdata);
...
#endif    ///TEMPLATE_MODULE_H_    //NOLINT