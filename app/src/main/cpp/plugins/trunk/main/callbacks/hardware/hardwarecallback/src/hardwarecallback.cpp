...
#include "../include/hardwarecallback.h"

#include "../../../public/include/common/commonenumdefine.h"

namespace ysos {
DECLARE_PLUGIN_REGISTER_INTERFACE(HardwareCallback, CallbackInterface);  ///<  插件的入口，一定要加上 //  NOLINT
HardwareCallback::HardwareCallback(const std::string &strClassName)
    : BaseThreadModuleCallbackImpl(strClassName) {
	logger_ = GetUtility()->GetLogger("ysos.hardware");
}

HardwareCallback::~HardwareCallback(void) {

}

int HardwareCallback::IsReady(void) {
  return YSOS_ERROR_SUCCESS;
}

int HardwareCallback::RealCallback(
